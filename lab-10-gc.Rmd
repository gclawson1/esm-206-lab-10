...
```{r blr}
donner_blr <- glm(Outcome ~ Age + Sex, family = "binomial", data = DonnerTable)

summary(donner_blr)
```
The odds of male surviving is lower than the odds of female survings. And younger people are more likely to survive. 
...
# this tells us there is an 81% chance of survival for a 5 year old female. 
```
...
#41% chance of survival for 25 year old male. 
```
...
age_probs <- predict(donner_blr, newdata = donner_newdata, type = "response", se.fit = TRUE)
# Coerce outcome into data frame. 
graph_data <- data.frame(donner_newdata, age_probs) %>%
...
```{r solar_data}
si_full <- list.files(pattern = "solar_irradiation_*") %>%   #create a list of .csv files
  map_df(~read_csv(.)) %>%    #read in the list of files. 
  clean_names()
...
solar_tidy <- si_full %>%
  ...
  filter(time != "NA") %>%
  mutate(site = fct_relevel(site, "Hawaii", "Santa Barbara", "Alaska"))  # put them in this order
...
aq_tidy <- aq_df %>%
  filter(country %in% c("Brazil", "Chile", "Ecuador", "United States")) %>%
...
```{r gganimate_fish}
aq_plot <- ggplot(aq_tidy, aes(x = year, y = aq_mil, group = country)) +
